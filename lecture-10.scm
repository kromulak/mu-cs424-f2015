...
;;; - conditionals like if (macro expand into closures & library function or λ-expressions)
...
(let ((count-nums0 (λ (count-nums00 s)
		     (cond ((number? s) 1)
...
(define curried-plus (λ (x) (λ (y) (+ x y))))

(define curry (λ (non-curried-binary-function) (λ (x) (λ (y) (non-curried-binary-function x y)))))
(define uncurry (λ (curried-binary-function) (λ (x y) ((curried-binary-function x) y))))

;; > (curry +)
;; #<procedure:...4/lecture-10.scm:49:47>
;; > ((curry +) 1)
...
;; want to convert g into recursive function.
;; let's call (Y g) that recursive function.
;; Want the following property: (g (Y g)) = (Y g)
;; In math, when   f(c) = c   we say that c is fixedpoint of f.
