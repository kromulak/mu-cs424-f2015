data Term = Term String [Term] | Hole LogicVariable -- its head is a string
			deriving (Eq, Show)
			
type LogicVariable = Int

type Unifier = [(LogicVariables,Term)]

unify :: Term -> Term -> Unifier -> Maybe Unifier

unify (Term aHead aArgs) (Term bHead bArgs) u
	= if aHead == bHead
		then unifyListts aArgs bArgs u
		else Nothing

unify (Hole lvA) b@(Term _bHead _bArgs) u = 
	maybe (Just ((lvA,b):u)) (\t->unify t b u) (lookup lvA u)
unify a@(Term _aHead _aArgs) b@(Hole lvb) u = unify b a u
unify a@(Hole lvA) b@(Hole lvB) u = 
	case (lookup lvA u, lookup lbB u) of
