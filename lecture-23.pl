...
%%% Type inference for System F (lambda calculus w/ "forall" types)

%% Representations.
%% Expression (aka lambda calculus term):

%%Term ::= LambdaExpr | Application | Variable | BasisObject
%%LambdaExpr ::= lambda Variable : Type . Term 
%%Application ::= Term Term
%%Variable ::= x1 | x2 | x3 | ...
%%BasisObject ::= b1 | b2 | b3 | ...

%% REPRESENT: lambda Variable : Type . Expr
...
%% AS: var(x1)m except in formal parameter of lambda expressions, where just: x1
...
%% (this is the "compose" combinatior)
...
%% Use G for type environment (G for capital gamma)

%% check if a term is well typed
wellTyped(E) :- hasTyped(E, T).
...
hasType(app(E1,E2), T2, G) :- 
		...
		hasType(E2,T1,G). % E1 is a function from T1 to T2, so the result of applying T1 to E2 = T2
hasType(var(V1), T, G) :- firstMember([x1,T],G). 	% first member only, ! = cut search tree NOW
hasType(basis(true), bool, _G). % we need the underscore to prevent the "singleton variables" warning
...
%%%
%%% yes
%%% | ?- hasType(lambda(f,_,lambda(g,_,lambda(x,_,app(var(f),app(var(g),var(x))))))).
%%%
